...
contract uTHOR is IERC4626, ERC20Vote, ReentrancyGuard {
    ...
    function withdraw(
        ...
        // burn receipt tokens and transfer $THOR
        _burn(owner, shares);
        address(_asset).safeTransfer(receiver, assets);

        // claim and transfer unclaimed $USDC rewards
        uint256 userRewards = accruedRewards[owner];
        accruedRewards[owner] = 0;
        address(_rewardAsset).safeTransfer(receiver, userRewards);

        emit Withdraw(msg.sender, receiver, owner, assets, shares);
        emit RewardClaimed(owner, userRewards);
    }
